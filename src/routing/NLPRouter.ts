...
export class NLPRouter {
  ...
  async handleNLPMessage(phone: string, userId: string, text: string, quotedMessageId?: string): Promise<void> {
    try {
      ...
      if (adaptedResult.confidence < requiredConfidence || adaptedResult.intent === 'unknown' || result.needsClarification) {
        ...
        // ===== GREETING DETECTION (before showing "I didn't understand") =====
        // Detect greetings and respond warmly instead of saying "I didn't understand"
        ...
        if (isGreeting) {
          logger.info('Greeting detected', { userId, text: normalizedText });
          ...
          await this.sendMessage(phone, `שלום${personalGreeting}! 👋\n\nאיך אפשר לעזור?\n\nאפשר לכתוב בשפה טבעית או לשלח /תפריט`);
          return;
        }
  ...
  private async handleNLPCreateEvent(phone: string, userId: string, intent: any, originalText: string): Promise<void> {
    ...
    try {
      ...
      // Add quick reminder creation tip
      successMessage += `\n\n💡 רוצה תזכורת? ענה להודעה זו עם:\n"תזכיר לי שעה לפני" או "תזכורת ב-20:00"`;
