...
1. CREATE EVENT WITH TIME: "קבע פגישה עם דני מחר ב-3" → {"intent":"create_event","confidence":0.95,"event":{"title":"פגישה עם דני","date":"2025-11-12T15:00:00+02:00","dateText":"מחר ב-3","contactName":"דני"}} (CRITICAL: include BOTH date ISO and dateText with time! CRITICAL: Extract "דני" to contactName from "עם דני" pattern!)
1a. CONTACT EXTRACTION WITH "עם" (CRITICAL - BUG FIX #24): "פגישה עם גדי" → {"intent":"create_event","confidence":0.95,"event":{"title":"פגישה עם גדי","contactName":"גדי"}} (CRITICAL: "עם X" = with X, extract X as contactName! Patterns: "עם גדי", "עם מיכאל", "עם הרופא")
...
1f. TIME vs DATE DISAMBIGUATION (CRITICAL - BUG FIX #22): "פגישה ב 21 עם דימה" → {"intent":"create_event","confidence":0.95,"event":{"title":"פגישה עם דימה","date":"<today 21:00 ISO>","contactName":"דימה"}} (CRITICAL: "ב X" where X is 0-23 = TIME today, NOT date! "ב 21" = at 21:00 TODAY, NOT 21st of month. Only treat as date if there's explicit month context like "21 באוקטובר" or "ב21 לחודש")
1g. MORE TIME DISAMBIGUATION EXAMPLES (CRITICAL): "אירוע ב 14" → today at 14:00, "פגישה ב 9" → today at 09:00, "מפגש ב 20" → today at 20:00 (CRITICAL: Single number 0-23 with ב prefix = TIME, not date!)
...
6c. LIST EVENTS BY DAY OF WEEK (CRITICAL - BUG FIX #23): "מה יש לי ביום רביעי?" → {"intent":"list_events","confidence":0.95,"event":{"dateText":"ביום רביעי"}} (CRITICAL: "מה יש לי ביום X" = list events on day X, NOT search for title "ביום רביעי"!)
6d. MORE DAY OF WEEK EXAMPLES (CRITICAL): "מה יש לי ביום ראשון" → list_events with dateText="ביום ראשון", "מה יש לי ביום שישי" → list_events with dateText="ביום שישי" (CRITICAL: Always use list_events, extract full "ביום X" to dateText!)
